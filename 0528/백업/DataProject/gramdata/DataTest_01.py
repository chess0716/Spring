...
# 'timestamp'와 '_id' 열 제거
df.drop(columns=['timestamp', '_id'], inplace=True)

# 결측치 대체
df.fillna(df.mean(), inplace=True)  # 수치형 데이터의 경우 평균값으로 대체
df.fillna('Unknown', inplace=True)  # 범주형 데이터의 경우 'Unknown'으로 대체
...
# 연령대 그룹화
df['age_group'] = pd.cut(df['age'], bins=[0, 18, 35, 55, np.inf], labels=['Teen', 'Young Adult', 'Adult', 'Senior'])

# 원-핫 인코딩
df = pd.get_dummies(df, columns=['age_group', 'environment'])

# 새로운 특성 생성
df['budget_per_frequency'] = df['budget'] / (df['frequency'] + 1)

# 모델 다시 학습
X_train, X_test, y_train, y_test = train_test_split(df.drop(['interests'], axis=1), df['interests'], test_size=0.2, random_state=42)
model.fit(X_train, y_train)
print("Updated Model Accuracy:", model.score(X_test, y_test))
